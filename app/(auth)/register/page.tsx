...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Successful registration, enter verification wait state
      setIsWaitingVerification(true);
      toast.success("Identity created. Please check your email.");
        ...
        >
          <div className="w-full max-w-[480px] relative">
            <motion.div 
              ...
              className="glass-panel rounded-[2.5rem] p-8 md:p-12 transition-all duration-300 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] relative overflow-hidden"
            >
                ...
                {!isWaitingVerification ? (
                  ...
                  >
                    ...
                    <form onSubmit={handleRegister} className="space-y-4" style={{ transform: "translateZ(40px)" }}>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-1">
                          <div className="flex justify-between items-center mb-1.5">
                            ...
                            <span className={`text-[9px] uppercase font-black ${passwordStrength < 2 ? 'text-red-400' : passwordStrength < 4 ? 'text-amber-400' : 'text-emerald-400'}`}>
                              {passwordStrength < 2 ? 'Vulnerable' : passwordStrength < 4 ? 'Standard' : 'Strong'}
                            </span>
                          </div>
                      ...
                      >
                        <span className="relative z-10 text-xs uppercase tracking-[0.2em]">{isLoading ? "Writing Record..." : "Register"}</span>
                      ...
                      <div className="text-center mt-8 pt-4 border-t border-white/[0.05]">
                        <p className="text-slate-500 text-xs font-medium">
                          Already registered? 
                          <Link href="/login" className="text-white hover:text-blue-400 font-bold transition-all ml-2 border-b border-white/20 hover:border-blue-400 pb-0.5">Proceed to Entry.</Link>
                        </p>
                      </div>
                    </form>
                  </motion.div>
                  ...
                  >
                    ...
                    <h2 className="text-white text-2xl font-bold tracking-tight text-center mb-4">Check your Email</h2>
