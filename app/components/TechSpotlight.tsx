...
export default function TechSpotlight() {
  return (
    <section>
      <h2 className="text-2xl font-bold text-dark-bg mb-4 border-l-4 border-[#4f46e5] pl-3">Tech Spotlight</h2>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div className="md:col-span-3 relative rounded-xl overflow-hidden min-h-[300px] group shadow-md">
          ...
          <div className="absolute bottom-0 left-0 p-6 w-full">
            ...
            <h3 className="text-2xl font-bold text-white mb-3 leading-tight max-w-[80%]">Can Artificial Intelligence Ever Truly Be Creative?</h3>
            <p className="text-gray-300 text-sm mb-4 line-clamp-2 max-w-[90%]">The shift from bits to qubits represents more than a speed upgrade - it&apos;s a fundamental change in...</p>
            <button className="bg-[#4f46e5] hover:bg-[#3730a3] text-white px-5 py-2 rounded-full text-sm font-medium inline-flex items-center gap-1.5 transition-colors shadow-lg">
              Read More
            </button>
          </div>
        </div>
        ...
        <div className="md:col-span-2 flex flex-col justify-between gap-4">
          {[
            ...
            <div key={i} className="flex gap-4 items-center group cursor-pointer hover:bg-gray-50 p-2 rounded-xl transition-all duration-300">
              <div className="w-20 h-20 rounded-xl relative overflow-hidden flex-shrink-0 shadow-sm">
              ...
              <div className="flex-1">
                <h4 className="font-bold text-dark-bg text-sm leading-snug group-hover:text-brand transition-colors mb-1">{item.title}</h4>
                <p className="text-[11px] text-gray-500 line-clamp-2 mb-1.5">{item.desc}</p>
                <p className="text-[9px] font-bold text-gray-400 uppercase">{item.time}</p>
              </div>
            </div>
          ))}
        </div>
