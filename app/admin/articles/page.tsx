...
export default function ArticlesManagement() {
  ...
  const fetchArticles = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("http://localhost:8001/public/articles");
      if (res.ok) {
  ...
  const deleteArticle = async (id: string) => {
    if (!confirm("Confirm article deletion? This cannot be undone.")) return;
    ...
    <div className="p-8 max-w-7xl mx-auto">
                 ...
                 {filteredArticles.map((article) => (
                    ...
                    >
                       <td className="px-8 py-6">
                          <div className="flex items-center gap-4 max-w-sm">
                             ...
                             <div className="truncate">
                                <p className="text-sm font-black text-slate-800 tracking-tight truncate hover:text-indigo-600 transition-colors cursor-pointer">{article.title}</p>
                       ...
                       <td className="px-6 py-6">
                          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${getStatusStyle(article.status)}`}>
                       ...
                       <td className="px-8 py-6 text-right">
                          <div className="flex justify-end gap-3">
                             ...
                             <button className="p-2.5 hover:bg-white hover:shadow-lg rounded-xl transition-all text-slate-400 hover:text-indigo-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-12">
          {[
            ...
            { label: 'Views Aggregate', val: articles.reduce((acc, a) => acc + (a.views_count || 0), 0), color: 'text-slate-900' }
          ].map((stat, i) => (
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-lg text-center">
