...
export default function SubscriptionPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div>
        <h1 className="text-2xl font-black text-slate-900 dark:text-white mb-1">Subscription</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">Manage billing period and autopay settings.</p>
      </div>
      ...
      {/* Usage Limits */}
      {usage && usage.features && Object.keys(usage.features).length > 0 && (
        <div className="mb-6 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Plan Usage</h3>
          <div className="space-y-4">
            {Object.entries(usage.features).map(([featureKey, featureData]: [string, any]) => {
              const isUnlimited = featureData.limit === -1;
              const isBlocked = featureData.limit === 0;
              const percent = (isUnlimited || isBlocked) ? 0 : Math.min(100, Math.round(((featureData.used || 0) / featureData.limit) * 100));
              const formatFeatureName = (key: string) => key.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                ...
                <div key={featureKey}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-bold text-slate-900 dark:text-white">
                      {isUnlimited ? "Unlimited" : isBlocked ? (
                        <Link href="/pricing" className="text-indigo-600 dark:text-indigo-400 hover:underline text-xs">
                          {isFree ? "Upgrade to Plus" : "Upgrade to Pro"}
                        </Link>
                      ) : (
                        <>
                          {`${featureData.used || 0} / ${featureData.limit}`}
                          <span className="text-xs text-slate-400 font-normal ml-1">
                            {featureData.window === 'monthly' ? '/mo' : featureData.window === 'rolling_24h' ? '/day' : ''}
                          </span>
                        </>
                      )}
                    </span>
                  ...
                  {(!isUnlimited && !isBlocked) && (
                    <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full ${percent >= 100 ? 'bg-red-500' : percent > 80 ? 'bg-amber-500' : 'bg-indigo-500'}`} 
                        style={{ width: `${percent}%` }}
                      ></div>
                    </div>
                  )}
      ...
      <div className="flex flex-col gap-3">
        ...
        {subscription?.cancel_at_period_end && (
          <Link
            href="/pricing"
            className="w-full px-5 py-3 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2"
          >
            <RefreshCw size={15} />
            Resubscribe to plan
          </Link>
